-- Database Schema for Vibe Truyện
...
-- Users (for favorites, reading history)
...
CREATE INDEX IF NOT EXISTS idx_stories_is_hot ON stories(is_hot);
...
-- Insert chapters for sample stories
INSERT INTO chapters (story_id, chapter_number, title, content, is_new)
SELECT 
  s.id,
  c.num,
  'Chương ' || c.num,
  'Nội dung chương ' || c.num || ' của ' || s.title || '. ' || 
  'Ánh trăng le lói qua khe cửa sổ, chiếu những vệt sáng mờ ảo xuống sàn nhà gỗ. ' ||
  s.title || ' - Phần ' || c.num || '. ' ||
  'Một câu chuyện đầy cảm xúc về tình yêu và sự hy sinh. ' ||
  'Tiếp tục theo dõi để khám phá những bí mật được giấu kín trong câu chuyện này. ' ||
  'Hãy đón đọc chương tiếp theo để hiểu thêm về số phận của các nhân vật.',
  c.num = s.chapters
FROM stories s
CROSS JOIN LATERAL (
